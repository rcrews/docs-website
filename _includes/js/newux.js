...
var NEWUX = (function ($) {
  ...
  var Pubnav = {
    ...
    makeLatestURL (url) {
      ...
      if (url.match(/HDPDocuments/)) { return urlParts.join('/') }
      urlParts[2] = 'latest'
  ...
  var Search = {
    ...
    launchSearch: function (evt) {
      ...
      var searchTerm = this.filterSearchTerm($(current).find('.searchterm').val())
    ...
    fireQuery: function (searchterm, nextCursorMark) {
      ...
      var fq = WhoAmI.search_name ? '(search-name:"' + WhoAmI.search_name + '" AND release:' + encodeURIComponent(that.formatReleaseNumber(WhoAmI.version.title)) + ')' : ''
      var rows = 10
      var params = {}
      var defaults = '&sort=score desc,id asc&facet=true&facet.field=product&facet.field=release&facet.field=booktitle&hl=true&hl.fl=text&fl=id,score,url,product,release,booktitle,title'
      ...
      this.query = $.ajax({
        ...
        jsonp: 'json.wrf',
        beforeSend: function () {
          if (!nextCursorMark) { $('.lucene-results .results').hide() }
          ...
          if (response.response.docs.length) {
            ...
            for (var book in outputHolder) {
              ...
              if (outputHolder.hasOwnProperty(book)) {
                ...
                for (var i = 0; i < outputHolder[book].length; i++) {
        ...
        .fail(function (jqXHR, textStatus) {
          ...
          var errMsg = '<h2><i class="fa fa-frown-o"></i> Uh-oh, the search request failed</h2>'
