...
class PubnavClass {
  ...
  setupNav() {
    ...
    if (!$('.collapse-ctoc').length) {
      $('.ctoc').append(`<div class="collapse-ctoc collapse">
        <i class="fa fa-angle-double-up"></i>
        </div>`);
    }
    if (!$('.bread-category').length) {
      $('.breadcrumbs').append('<span class="bread-category"></span>');
    }

    // if (!$(".short-prev").length) {
    //   $(".cpage").append('<div class="short-prev"><a href="">«</a></div>');
    // }
    if (!$('.short-next').length) {
      $('.cpage').append('<div class="short-next"><a href="">»</a></div>');
    }
    // if (!$("up").length) {
    //   $(".cpage").append('<div class="up"></div>');
    // }
    // if (!$(".prev").length) {
    //   $(".cpage").append('<div class="prev"><a href=""></a></div>');
    // }
    if (!$('.next').length) {
      $('.cpage').append('<div class="next"><a href=""></a></div>');
    }
    ...
    if (!$('.copyright').length) {
      $('.cpage').append(`<div class="copyright">
        <a href="/common/html/legal.html">${this.pagestate.copyright}</a>
        </div>`);
    }

    // When we load up the JS for the page, we rely on the
    // navigation.json file that is present in the product root.

    // Normally, we expect this to be in the third folder... so for
    // example. /HDF3/hdf-3.0.4/navigation.json
    ...
    // Now, get the JSON, and then build out a shadow structure and
    // insert it into the DOM.
    // TODO!!! - Handle errors?
    fetch(navfile)
        .then(resp => resp.json()) // Transform the data into json
        .then(data => {
          // 1. Clean and build tree... add unique ids for all elements
          // based on hrefs, check for duplicates.
          ...
          // TODO! - We're looking for the page based on an id which we
          // made up, but really we should just base it on the href
          ...
          // 4... Figure out the PDF... TODO!! - This is repeated in
          // LoadContent() - Make it DRYer)
  ...
  loadContent(url, hash, updateHistory) {
    ...
    // $(".maincontent").append(`<div id="content-spinner">
    ...
    jQuery.ajax({
      ...
      // Parse the response..
      // First, create a virtual DOM that we can set the location
      // correctly with.
      ...
      if (!elems.length) {
        if (Pubnav.pagestate.children.length && Pubnav.clicktrack < 3) {
          // sometimes foyer pages are empty. If so, look for the first
          // child page.
          const newUrl = Pubnav.pagestate.children[0].href;
          Pubnav.requestNewPage(newUrl);
          return false;
        } else {
          // Page doesn't have kids, and still doesn't have content...
          // maybe it should be external?
          elems = [];
          elems[0] = `<h1>Format error</h1>
            <p>We found the page you requested but can't identify the
            main content of the page to render within this page. <a
            href="${url}" class="external">Try loading it directly.</a>
            Otherwise, choose a different topic from the left or find
            one using search.</p>`;
          // Fire GA Logger?
          complete = true;
          swapContent();
        ...
        // Update history so the back button works.... We don't want
        // this to fire if we're going back in time!
        if (updateHistory) {
        ...
        if (typeof copyright !== 'undefined') {
          ...
          Pubnav.pagestate.copyright = `&copy; ${new Date().getFullYear()}
            by Cloudera, Inc. All rights reserved.`;
        }
    ...
    for (let i = 0; i < data.length; i++) {
      if (typeof data[i].id === 'undefined') {
        if (typeof data[i].href !== 'undefined') {
          ...
          // items that have no page.
          data[i].id = Utils.slugify(data[i].text);
        }
      } else {
        // We should still clean the ID.. just in case there are
        // periods, slashes and stuff in it.
        data[i].id = data[i].id.replace(/([^a-z0-9-]+)/gi, '-');
      }
      // Now the ID has been defined... if the item has kids, let's
  ...
  contractNavElem(id) {
    ...
    // Again the hack to hide the item after compression.... I think we
    // could do this with keyframes instead.
  ...
  requestNewPage(url, hash, updateHistory) {
    ...
    if (destination) {
      ...
      // Watch out. Everything after this, will run before the content
        ...
        if (typeof navarray[i].sub === 'object') {
          if (this.mapPageState(navarray[i].sub, id)) { // If the kid was it!
            if (i + 1 < navarray.length && Utils.isEmpty(this.pagestate.next)) {
              // This sets the next page at the parent level by
              // default.. which will probably get overridden again at
              // the lower level.
              this.pagestate.next = navarray[i + 1];
            }
  ...
  updatePageState() {
    ...
    if (typeof this.pagestate.next === 'object' && !Utils.isEmpty(this.pagestate.next)) {
      // Edge case... sometimes the "next" item might be a parent
      // category... without a URL.. in which case, the first subchild
      // is probably what we want.
      if (typeof this.pagestate.next.href === 'undefined' &&
          typeof this.pagestate.next.sub === 'object') {
        this.pagestate.next = this.pagestate.next.sub[0];
      }
    ...
    // I've modified this so that the topic is the only breadcrumb at
    ...
    if (this.pagestate.breadpath.length >= 2) {
      // Only if it's a topic title. Breadpath[0] is likely to be a
      // category. Breadpath[1] Might be the topic title.
