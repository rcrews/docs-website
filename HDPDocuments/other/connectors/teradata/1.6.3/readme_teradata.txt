Copyright (c) 2019 Teradata Corporation.
ALL RIGHTS RESERVED
...
System Requirements:
    1. JRE/JDK (OracleJDK or OpenJDK)
...
Contents
   ...
   10.0 AWS Recommendations
   11.0 Data Compression
   12.0 Hadoop Port Requirements
   13.0 Teradata Connectivity Recommendations
...
2.0 Release Content
    ...
    2.1 Supported Functionalities
        ...
        6. Compression
           ...
           b) Export -> Teradata
              ...
              Only Snappy intermediate compression
	...
	Included features in the 1.5.4 release
        ...
        Included features in the 1.5.0 release
         ...
         TDCH-1292: configureOozie script has been modified to copy needed jars from HDFS instead of from local
                    system for HDP 2.x
        ...
        Included features in the 1.4.3 release
         ...
         TDCH-1199: Improve performance of retrieving table partitions for
                    split.by.partition
         TDCH-1161: Add support for conversion from TD TIME to Hive TIMESTAMP
                    on import
         TDCH-1133: Add TDCH argument to use INSERT OVERWRITE instead of INSERT
                    INTO to overwrite the data that is already in the hive table
                    during an import
	 ...
	 Included fixes in the 1.5.4 release
	  ...
	  TDCH-1657: [CDH] Integrate new rmHA command-line argument into any installation scripts that 
         ...
         Included fixes in the 1.5.3 release
          ...
          TDCH-1533: TDCH silently succeeds with wrong values while using HDFS export for non-bounded NUMERIC data type
                     in target (Teradata DB)

         Included fixes in the 1.5.2 release

          TDCH-685:  Internal.fastload export jobs with target tables that include VARCHAR cols with length >= 64k
                     result in SQLException
          ...
          TDCH-1465: ORC import giving NULL value instead of expected 0 length with split.by.amp method for 0 length
                     VARCHAR values
         ...
         Included fixes in the 1.5.0 release
          ...
          TDCH-1259: TDCH gives index out of bounds for string to timestamp export if timeformat uses more than
                     3 digits in nanoseconds
         ...
         Included fixes in the 1.4.3 release
          ...
          TDCH-742: Null pointer exception thrown when column names that include
                    spaces are double quoted in targettableschema value
          TDCH-577: Method used for map tasks to connect to client for
                    internal.fastload can pick wrong address
...
4.0 Installation/Uninstallation
    ...
    4.1 Dependencies
        ...
        In order to run Hive jobs, Teradata Connector for Hadoop requires
	...
	Atlas jar requirements:
        Hadoop 3.0
             a) atlas-common-1.0.0.3.0.1.0-187.jar
             b) atlas-intg-1.0.0.3.0.1.0-187.jar
    ...
    4.2 Uninstallation of TDCH releases prior to 1.4.x
        ...
        also had the user copy the TDCH jar file, and other accompanying drivers, into
        the /usr/lib/hadoop/lib directory (or another related hadoop directory).
    ...
    4.3 Installation of 1.4.x and above
        ...
        5. If the node that TDCH is installed on has a Java SDK < 1.8 the following
           should be modified to fix an issue with JDBC slow logon problems.

           Modify mapreduce.map.java.opts property value in mapred-site.xml on
           ...
           Teradata Connector job command:
             ...
             -Dmapreduce.map.java.opts="-Djava.security.egd=file:/dev/./urandom"
...
5.0 Plugin Design
    ...
    5.3 Plugin Usage
        ...
        5.3.2 Common
            ...
            19. tdch.teradata.unicode.passthrough

                Description:      Enable Unicode Passthrough for the JDBC driver
                                  (only available when using a Teradata Database
                                  16.00+)
                Required:         no
                Supported values: true|false
                Default value:    false
        ...
        5.3.3 Teradata Source
            ...
            18. tdch.input.teradata.stage.table.name
            ...
            20. tdch.input.teradata.split.by.column
            ...
            21. tdch.input.teradata.stage.table.blocksize
                Description:      The blocksize to be applied when creating
                                  staging tables.
                Required:         no
                Supported values: an integer greater than 0
	    ...
	    15. tdch.output.teradata.stage.table.name
                Description:      The name in Teradata system with which
                                  Teradata Connector for Hadoop uses to create
                                  staging table, if staging is required. The
                                  staging table should not be existed in the
            ...
            17. tdch.output.teradata.stage.table.kept
            ...
            18. tdch.output.teradata.fastload.coordinator.socket.host
            ...
            22. tdch.output.teradata.error.table.name
            ...
            25. tdch.output.teradata.stage.table.blocksize
                Description:      The blocksize to be applied when creating
                                  staging tables.
                Required:         no
                Supported values: an integer greater than 0
            ...
            26. tdch.output.teradata.truncate
                Description:      If this parameter is set to true it will delete the
                                  data in the Teradata table before transferring.
                Required:         no
                Supported values: true|false
                Default value:    false

            27. tdch.output.hdfs.logging
                Description:      Turn on HDFS logging for exception logging for transfer
                                  from HDFS/Hive to Teradata DB
                Required:         no
                Supported values: true|false
                Default value:    false

            28. tdch.output.hdfs.logging.path
                Description:      Path on HDFS to store log if logging is enabled
                Required:         yes if logging is enabled
                Supported values: String
                Default value:

            29. tdch.output.hdfs.logging.delimiter
                Description:      Delimiter character between columns for HDFS log
                Required:         no
                Supported values: String
                Default value:    |

            30. tdch.output.hdfs.logging.limit
        ...
        5.3.8 Hive Target
            ...
            11. tdch.output.hive.overwrite
                Description:      If this parameter is set to true and the Hive
                                  table is non-partitioned it will overwrite the
                                  entire table.  If the table is partitioned the
                                  data in each of the affected partitions will
                                  be overwritten and the partition data that is
...
6.0 Command-Line Parameters
    ...
    6.2 ConnectorImportTool/ConnectorExportTool common command-line parameters
         ...
         2. fileformat
            ...
            Required:         no
            Supported values: orc|rcfile|sequencefile|textfile|avrofile|parquet
         ...
         3. method
            ...
            Supported values: split.by.hash|split.by.partition|split.by.value
                              split.by.amp|internal.fastexport|batch.insert|internal.fastload
        ...
        37. upt
            Configuration:    tdch.teradata.unicode.passthrough

        38. replaceunsupportedunicodechars
	...
	16. stagetablename
        ...
        18. numpartitionsinstaging
            Configuration:    tdch.input.teradata.num.partitions

        19. fastexportsockethost
            Configuration:    tdch.input.teradata.fastexport.coordinator.socket.host

        20. fastexportsocketport
            Configuration:    tdch.input.teradata.fastexport.coordinator.socket.port

        21. fastexportsockettimeout
            Configuration:    tdch.input.teradata.fastexport.coordinator.socket.timeout

        22. targetcompression
            Description:      Enable intermediate, output or both target compression
                              (required for Hive and HDFS jobs for enabling compression)
            Required:         no
            Supported values: I|O|B
                              I: intermediate
                              O: output
                              B: both
            Default value:    none
        ...
        23. targetcompressiontype
            Description:      Set the target compression type
                              (required for Hive and HDFS jobs for enabling compression)
            Required:         no (yes if target compression is enabled)
            Supported values: BLOCK|RECORD|NONE
            Default value:    NONE
            Case sensitive:   yes

        24. targetcompressioncodec
            Description:      Set the target compression codec
                              (required for Hive and HDFS jobs for enabling compression)
            Required:         no (yes if target compression is enabled)
	...
	15. stagetablename
        ...
        22. sourceintermediatecompression
            Description:      Enable source intermediate compression (Snappy)
            Required:         no
            Supported values: true|false
            Default value:    false
            Case sensitive:   no
        ...
        23. hdfslogenable
            Configuration    tdch.output.hdfs.logging

        24. hdfslogpath
            Configuration    tdch.output.hdfs.logging.path

        25. hdfslogdelimiter
            Configuration    tdch.output.hdfs.logging.delimiter

        26. hdfsloglimit
            Configuration    tdch.output.hdfs.logging.limit
...
7.0 Basic Usage
    ...
    7.1 Basic Usage
        ...
        7.1.1 Data movement
            ...
            vars should be updated according to the Hive version found in your
            environment; see section 4.1
             ...
             export HADOOP_CLASSPATH=$HIVE_HOME/conf:
                           ...
                           $HIVE_HOME/lib/datanucleus-rdbms-4.1.17.jar:
                           $HIVE_HOME/lib/hive-cli-3.1.0.3.0.1.0-187.jar:
                           $HIVE_HOME/lib/hive-exec-3.1.0.3.0.1.0-187.jar:
                           $HIVE_HOME/lib/hive-jdbc-3.1.0.3.0.1.0-187.jar:
                           $HIVE_HOME/lib/hive-metastore-3.1.0.3.0.1.0-187.jar:
                           $HIVE_HOME/hive-service-3.1.0.3.0.1.0-187.jar:
                           $HIVE_HOME/lib/jdo-api-3.0.1.jar:
                           $HIVE_HOME/lib/libfb303-0.9.3.jar:
                           $HIVE_HOME/lib/libthrift-0.9.3.jar:
                           $HIVE_HOME/lib/hive-llap-tez-3.1.0.3.0.1.0-187.jar:
                           $HIVE_HOME/lib/calcite-core-1.16.0.3.0.0.0-1634.jar:
                           $HIVE_HOME/lib/log4j-api-2.10.0.jar:
                           $HIVE_HOME/lib/hadoop-lzo-0.6.0.3.0.1.0-187.jar:
                           $TEZ_HOME/lib/slf4j-api-1.7.10.jar:
                           ${ATLAS_HOME}/atlas-common-1.0.0.3.0.1.0-187.jar:
                           ${ATLAS_HOME}/atlas-intg-1.0.0.3.0.1.0-187.jar:
                           ${ATLAS_HOME}/atlas-notification-1.0.0.3.0.1.0-187.jar:
                           ${ATLAS_HOME}/commons-configuration-1.10.jar:
                           ${ATLAS_HOME}/hive-bridge-1.0.0.3.0.1.0-187.jar

            export LIB_JARS=$HIVE_HOME/lib/hive-cli-3.1.0.3.0.1.0-187.jar,
                           $HIVE_HOME/lib/hive-exec-3.1.0.3.0.1.0-187.jar,
                           $HIVE_HOME/lib/hive-metastore-3.1.0.3.0.1.0-187.jar,
                           $HIVE_HOME/lib/libfb303-0.9.3.jar,
                           $HIVE_HOME/lib/libthrift-0.9.3.jar,
    ...
    7.2 Advanced Usage
        ...
        7.2.1 Advanced DateTime Type Conversions
           ...
           packaged with TDCH 1.4.x and above by default.
        ...
        7.2.2 User defined converters      
          ...
          The "-sourcetableschema" command-line argument is required when reading from
          HDFS and not Hive where the table schema is already available.
        ...
        7.2.3 Plugin parameter usage
           ...
           export HADOOP_HOME=/usr/hdp/3.0.1.0-187/hadoop
           export HIVE_HOME=/usr/hdp/3.0.1.0-187/hive
           export SQOOP_HOME=/usr/hdp/3.0.1.0-187/sqoop
           export TDCH_JAR=/usr/lib/tdch/1.6/lib/teradata-connector-1.6.3.jar

           export HADOOP_CLASSPATH=$SQOOP_HOME/lib/avro-1.8.1.jar:
                          $SQOOP_HOME/lib/avro-mapred-1.8.1-hadoop2.jar:
                          $SQOOP_HOME/lib/paranamer-2.7.jar

           export LIB_JARS=$SQOOP_HOME/lib/avro-1.8.1.jar,
                          $SQOOP_HOME/lib/avro-mapred-1.8.1-hadoop2.jar,
                          $SQOOP_HOME/lib/paranamer-2.7.jar
...
9.0 Limitations/Issues
    ...
    9.1 Teradata Connector for Hadoop
       ...
       General Limitations
          ...
          a) When using the 'split.by.amp' method, all empty (zero length) strings
             in Teradata Database will be converted to NULL when imported in Hadoop
          b) When using the 'split.by.value' method, the split-by column cannot contain
             NULL values
	     ...
	     For non-kerberos environment you can set environment variable HADOOP_USER_NAME=hive
          f) This release TDCH 1.6.x supports only HDP 3.0 and later versions.
    ...
    9.3 Hive
     ...
     9.5 Parquet
         ...
         c) For HDP 2.5.x, HDP 2.6 and EMR, Parquet support for Hive import is not supported
            (only export from Hive is supported)
         d) The Hive TIMESTAMP data type is not supported but the Teradata Database
            TIMESTAMP data type is supported if the corresponding column in Hive is
            either VARCHAR or STRING data type
	 ...
	 TD Wallet creates temporary files in /tmp directory path by default. To change
         the /tmp directory path below environment variable needs to be set.
	   export _JAVA_OPTIONS=-Djava.io.tmpdir=/<yourowntmpdir>

10.0 Data Compression

    TDCH 1.5.2 added the ability to compress the output data when importing into
    Hadoop or intermediate compression to compress the data during the MapReduce
    job.

    For TDCH Export (Hadoop -> Teradata), only intermediate compression is
    available and the intermediate compression will always use the Snappy codec.

    For TDCH Import (Teradata -> Hadoop), the following codecs are supported for
    output:
    ...
    And the Snappy codec is always used for intermediate compression (if enabled).

    For more information on compression in Hadoop see the following link:

    http://comphadoop.weebly.com/

11.0 Hadoop Port Requirements

    TDCH requires that all nodes in the Hadoop cluster have port 1025 open for both
    inbound and outbound traffic.

    If using internal.fastload and/or internal.fastexport, all ports between 8678 and
    65535 should be open for inbound and outbound traffic between all nodes of the
    Hadoop cluster.  The protocols start at port 8678 and look for an open port on
    the coordinator node (the node where the TDCH job is started from) and go up
    to port 65535 until they find an open port to communicate with each mapper.  The
    command line arguments "-fastloadsocketport" (for internal.fastload) and
